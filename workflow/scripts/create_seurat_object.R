...
#######################################################################
    ...
    ## tokeep <- which(s.subset@assays$RNA@counts %>% apply(1, sum) > 10) 
    tokeep <- which(s.subset@assays$RNA@counts %>% apply(1, function(x) ((x > 0) %>% as.numeric %>% sum > 10))) # keep genes detected in more than 10 UMIs
