...
option.list <- list(
    make_option("--figdir", type="character", default="/oak/stanford/groups/engreitz/Users/kangh/TeloHAEC_Perturb-seq_2kG/211119_make_sample_input_table/figures/", help="Figure directory"),
    make_option("--outdir", type="character", default="/oak/stanford/groups/engreitz/Users/kangh/TeloHAEC_Perturb-seq_2kG/211119_make_sample_input_table/outputs/", help="Output directory"),
...
geneNameConversion <- read.delim("../data/heterogenous_geneName_conversion_toMatchENSGID.txt", stringsAsFactors=F) %>%
...
ptb10xNames.df <- read.delim("../data/220627_add_Perturbation 10X names.txt", stringsAsFactors=F, check.names=F)
...
ProgramsRegulatedByThisGene.df <- MAST.df %>% subset(fdr.across.ptb < ptb.topic.thr) %>%
    ...
    mutate(PreviousConversionGene = convert.gene.names.toMatchENSGID(perturbation),
           OriginalGene = perturbation,
           Gene = perturbseq.gene.names.to10X(perturbation),
           ProgramID = gsub("topic_", "K60_", primerid)) %>%
    mutate(EntrezID = symbol.to.entrez[.$Gene %>% as.character] %>% sapply("[[",1) %>% as.character) %>%
    mutate(ENSGID = entrez.to.ensembl[.$EntrezID %>% as.character] %>% sapply("[[", 1) %>% as.character)
...
ProgramsInWhichGeneIsExpressed.df <- theta.zscore.rank.df %>%
    ...
    add.EntrezID.ENSGID %>%
    as.data.frame
IncNMFAnalysis.tokeep <- ProgramsInWhichGeneIsExpressed.df %>%
    select(Gene, OriginalGene, PreviousConversionGene, EntrezID, ENSGID) %>%
    unique %>%
    mutate(IncNMFAnalysis = TRUE) %>%
    as.data.frame

## ProgramsInWhichGeneIsExpressed.df <- theta.zscore %>%
##     `colnames<-`(paste0("K60_", colnames(.))) %>%
##     as.data.frame %>%
##     mutate(Gene = rownames(.)) %>%
##     melt(id.vars="Gene", variable.name="ProgramID", value.name="Topic.zscore")
##     ## mutate(Gene = convert.gene.names.toMatchENSGID(rownames(.))) %>%
##     mutate(PreviousConversionGene = convert.gene.names.toMatchENSGID(rownames(.))) %>%
##     mutate(Gene = perturbseq.gene.names.to10X(rownames(.)))
...
ProgramsInWhichGeneIsInTopNListZScoreSpecificGenes.tokeep <- Reduce(
    ...
    lapply(params.top.n.genes.in.topic.list, function (params.top.n.genes.in.topic) { # for each selected number of top genes we want to include
...
list.all <- list(ProgramsRegulatedByThisGene = ProgramsRegulatedByThisGene.tokeep,
                 ...
                 ## Top5ProgramsThatContributeToPoPSScore = Top5ProgramsThatContributeToPoPSScore.tokeep,
                 ## Top5FeaturesThatContributeToPoPSScore = Top5FeaturesThatContributeToPoPSScore.tokeep,
...
## df.all <- Reduce(function(x, y, ...) full_join(x, y, by = c("Gene", "ENSGID", "EntrezID"), ...), list.all) %>%

## df.all <- Reduce(function(x, y, ...) full_join(x, y, by = c("Gene", "OriginalGene", "PreviousConversionGene", "ENSGID", "EntrezID"), ...), list.all) %>%
df.all <- Reduce(function(x, y, ...) full_join(x, y %>% select(-OriginalGene, -PreviousConversionGene), by = c("Gene", "ENSGID", "EntrezID"), ...), list.all) ## %>%
    ## mutate(Gene = convert.gene.names.toMatchPerturbseq(Gene))
    ## mutate(Gene = perturbseq.gene.names.to10X(Gene))
...
df.all <- df.all %>% subset(!(is.na(Gene) & is.na(ProgramsRegulatedByThisGene) & !(Gene %in% (theta.zscore %>% rownames)))) %>% ## combine duplicated Gene names' rows
    ...
    as.data.frame
## df.all <- df.all[-na.row.index,]

## write table
write.table(df.all, file=paste0("./outputs/sample.cNMF.PoPS.input.to.large.table.pipeline.txt"), row.names=F, quote=F, sep="\t")
