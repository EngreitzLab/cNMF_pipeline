...
option.list <- list(
    ...
    make_option("--ABC.enhancers", type="character", default="/oak/stanford/groups/engreitz/Users/kangh/V2G2P_HCASM/data/SMC_encode_e2g_predictions_qnorm_filtered.ENCODE-rE2G.Score.qnorm.0.146.uniq.bed", help="ABC or scE2G results location for motif to gene matching"),
    make_option("--expressed.genes", type="character", default="pval0.0001", help="threshold for subsetting motif matches"),
...
## opt$ABC.enhancers <- "/oak/stanford/groups/engreitz/Users/kangh/V2G2P_HCASM/data/SMC_encode_e2g_predictions_qnorm_filtered.ENCODE-rE2G.Score.qnorm.0.146.bed"

## ## HCASM V2G2P all_genes
## opt$sampleName <- "HCASM.library"
## opt$ABC.enhancer <- "/oak/stanford/groups/engreitz/Users/kangh/V2G2P_HCASM/data/SMC_encode_e2g_predictions_qnorm_filtered.ENCODE-rE2G.Score.qnorm.0.146.bed"
## opt$organism <- "human"
## opt$outdir <- "/oak/stanford/groups/engreitz/Users/kangh/V2G2P_HCASM/250310_snakemake_HCASM_NovaSeq2/analysis/all_genes"
## opt$motif.promoter.background <- "/oak/stanford/groups/engreitz/Users/kangh/V2G2P_HCASM/250310_snakemake_HCASM_NovaSeq2/analysis/all_genes/HCASM.library/fimo/fimo_out/fimo.tsv"
## opt$ABC.enhancers <- "/oak/stanford/groups/engreitz/Users/kangh/V2G2P_HCASM/data/multiome_powerlaw_v3/encode_e2g_predictions_threshold0.149.uniq.bed"
## opt$expressed.genes <- "/oak/stanford/groups/engreitz/Users/kangh/V2G2P_HCASM/250310_snakemake_HCASM_NovaSeq2/analysis/data/HCASM.library.h5ad.all.genes.txt"


## ## HCASM V2G2P top 2000 most variable genes
## opt$sampleName <- "HCASM.library"
## opt$outdir <- "/oak/stanford/groups/engreitz/Users/kangh/V2G2P_HCASM/250118_snakemake_HCASM_top2000VariableGenes/analysis/top2000VariableGenes"
## opt$ABC.enhancers <- "/oak/stanford/groups/engreitz/Users/kangh/V2G2P_HCASM/data/SMC_encode_e2g_predictions_qnorm_filtered.ENCODE-rE2G.Score.qnorm.0.146.bed"
## opt$organism <- "human"
## opt$motif.promoter.background <- "/oak/stanford/groups/engreitz/Users/kangh/V2G2P_HCASM/250118_snakemake_HCASM_top2000VariableGenes/analysis/top2000VariableGenes/HCASM.library/fimo/fimo_out/fimo.tsv"
## opt$expressed.genes <- "/oak/stanford/groups/engreitz/Users/kangh/V2G2P_HCASM/250118_snakemake_HCASM_top2000VariableGenes/analysis/data/HCASM.library.h5ad.all.genes.txt"
...
motif.background.filePath <- paste0(ifelse(opt$motif.promoter.background!="", opt$motif.promoter.background, "/oak/stanford/groups/engreitz/Users/kangh/2009_endothelial_perturbseq_analysis/topicModel/2104_remove_lincRNA/data/fimo_out_all_promoters_thresh1.0E-4/fimo.tsv"))
motif.background <- read.delim(file=motif.background.filePath, stringsAsFactors=F) ## %>% filter(!grepl("#", motif_id))  # 30 second


## remove rows with NA entries
motif.background <- motif.background %>% subset(!is.na(sequence_name) | sequence_name != "NA")
...
if(grepl("[|]", motif.background$sequence_name[1])) {
    motif.background <- motif.background %>%
        separate(col="sequence_name", into=c("sequence_type", "gene_region", "Gene", "ENSGID"), sep="[|]", remove=F)
    ## if(ep.type == "promoter") {
    ##     motif.background <- motif.background %>% filter(sequence_type == "promoter")
    ## } else {
    ##     motif.background <- motif.background %>% filter(sequence_type != "promoter")
    ## }
}

## motif.background <- motif.background %>% subset(!is.na(p.value)|p.value !="NA") ## remove header lines

## load expressed genes
expressed.genes <- read.delim(opt$expressed.genes, stringsAsFactors=F, header=F) %>% as.matrix %>% as.character #
...
motif.background <- motif.background %>% subset(Gene %in% expressed.genes) 
...
## ## sequence regions to genes
## E2G.match.df <- ABC.df %>% select(sequence_name, Gene) %>% unique %>% subset(Gene %in% expressed.genes)

## motif.background <- motif.background %>% merge(E2G.match.df, by="sequence_name", all.x=T)

## save the files
write.table(motif.background, paste0(OUTDIR, "/", SAMPLE, "/fimo/motif.background.txt"), sep='\t', quote=F, row.names=F)
